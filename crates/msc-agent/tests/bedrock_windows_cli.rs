//! P10.15's offline Windows CLI-to-contract proof.
//!
//! This uses the real `msc` binary against a disposable loopback HTTP server.
//! It does not download or start a real Bedrock Dedicated Server.
        ...
        ("GET", "/v1/capabilities") => json!({
            ...
            "serverTypes": {
                ...
                "bedrock": {
                    ...
                    "backend": null,
                    "state": "unavailable",
                    "reason": "no-tested-windows-bds-package"
                }
            },
...
async fn windows_cli_drives_lifecycle_and_reads_unavailable_capability() {
    ...
    assert_eq!(
        serde_json::from_slice::<Value>(&status.stdout).unwrap()["serverType"],
        "bedrock"
    );
    assert!(cli(&url, &["server", "start"]).status.success());

    let command = cli(&url, &["command", "say hello from windows cli"]);
    assert!(
    ...
    assert_eq!(recorded.commands, vec!["say hello from windows cli"]);
