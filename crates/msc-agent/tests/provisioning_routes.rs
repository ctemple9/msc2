...
//! `world_backup_routes.rs` is: `build_app()` unconditionally provisions
//! a real production `SecretStore`, and only macOS Keychain is
//! available, isolated, and fast enough in this environment to exercise
//! that path in a test.
...
#[test]
fn provisioning_routes_are_mounted_behind_bearer_auth() {
    let temp = temp_dir("provisioning-routes-mounted");
  ...
  "servers": []
    ...
    let keychain_service = format!(
        "com.msc2.provisioning-routes-mounted.{}.{}",
        std::process::id(),
        suffix()
    );
    ...
    for path in [
        ...
        assert!(
            response.starts_with("HTTP/1.1 401"),
            "{path} POST expected 401 (mounted + auth-gated), got: {}",
            response.lines().next().unwrap_or_default()
        );
    }
    ...
    let response = http_get(port, "/v1/servers/does-not-exist-route", None);
    assert!(
...
#[test]
fn provisioning_routes_accept_the_bootstrap_token() {
    let temp = temp_dir("provisioning-routes-authed");
    let data_dir = temp.join("data");
    let servers_root = temp.join("servers");
    let config_path = data_dir.join("server_config_swift.json");
    fs::create_dir_all(&data_dir).unwrap();
  ...
  "servers": []
    ...
    let base_url = format!("127.0.0.1:{port}");
    let keychain_service = format!(
        "com.msc2.provisioning-routes-authed.{}.{}",
        std::process::id(),
        suffix()
    );
    let log_path = temp.join("agent.log");
    ...
    // gate, and not just JSON-shape rejection) runs.
    let response = http_post_json(port, "/v1/servers/create", Some(TOKEN), r#"{"name":"   "}"#);
    assert!(
        response.starts_with("HTTP/1.1 400"),
        "authenticated blank-name create expected 400, got: {}",
        response.lines().next().unwrap_or_default()
    );
