//! Native Windows Bedrock Dedicated Server runtime.
//!
//! Windows uses the same portable lifecycle as Linux, but the real process
//! supervisor assigned to this adapter is the Windows Job Object supervisor.
//! That keeps force-stop and service-owned process-tree cleanup in the
//! platform crate instead of making the shared runtime depend on Windows APIs.
...
/// The Windows runtime can be constructed on any host for capability
/// reporting, but only Windows may claim a native BDS implementation.
pub struct WindowsBedrockRuntime<'supervisor, C = SystemBedrockRuntimeClock> {
...
impl<'supervisor, C: BedrockRuntimeClock> WindowsBedrockRuntime<'supervisor, C> {
    pub fn with_host(
        process_supervisor: &'supervisor dyn ProcessSupervisor,
    ...
    pub fn launch_request(&self) -> Option<ProcessSpawnRequest> {
        self.server_dir
            .as_deref()
            .map(bedrock_native::windows_bedrock_spawn_request)
    }
...
impl<C: BedrockRuntimeClock> BedrockRuntime for WindowsBedrockRuntime<'_, C> {
    ...
    fn start(&mut self, request: BedrockStartRequest) -> Result<(), BedrockRuntimeError> {
        ...
        let pid = self
            .process_supervisor
            .spawn(bedrock_native::windows_bedrock_spawn_request(server_dir))
            .map_err(|error| BedrockRuntimeError::Transport(error.to_string()))?;
...
pub type NativeWindowsBedrockRuntime<'supervisor> = WindowsBedrockRuntime<'supervisor>;
