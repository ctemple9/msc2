//! Real Java process supervision for macOS.
//!
//! P4.9 intentionally made the application boundary synchronous, so this
//! module feeds that contract with background reader/waiter threads. The
//! child is placed in its own POSIX process group so a forced stop reaches
//! Java's helper children, not just the first JVM process.
...
pub struct MacosJavaProcessSupervisor {
...
impl MacosJavaProcessSupervisor {
    pub fn new() -> Self {
        Self::default()
    }
}

impl ProcessSupervisor for MacosJavaProcessSupervisor {
    fn spawn(&self, request: ProcessSpawnRequest) -> Result<ProcessId, ProcessError> {
        ...
        // It does not touch Rust-owned memory and only asks the OS to make the
        // child the leader of a new process group.
        unsafe {
            command.pre_exec(|| {
...
mod tests {
    ...
    // failing yet.
    fn drain_until_exit(
        supervisor: &MacosJavaProcessSupervisor,
        pid: ProcessId,
    ) -> Vec<ProcessEvent> {
    ...
    fn process_supervisor_real_streams_stdin_stdout_stderr_and_exit_code() {
        let supervisor = MacosJavaProcessSupervisor::new();
        let pid = supervisor
            .spawn(request(
