...
## 5. Router guides, concept guide, onboarding

D-026 point 3 keeps the router guide catalog as data separate from its matcher/resolver/composer (executable behavior, ported to Rust, not content). Router guide steps and concept-guide topics use the same Markdown-with-front-matter format and the same embedded-content storage as handbook topics — they are additional `content/help/**/*.md` files, not a second format. They are not required to carry a `helpId` themselves (nothing points *into* them from a DTO field the way `settings.difficulty` points from `SettingFieldDTO`); they're reached by their own catalog/topic list routes, out of scope for this step and left to whichever later step designs the handbook/router-guide routes proper. This step only defines the pointer mechanism (`helpId` → `GET /v1/help/{helpId}`) that DTOs use to reach into that content.
