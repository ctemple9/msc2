...
- **The formal world-slot model** — stays **Phase 6**. Raw import copies and labels world data but does not create a slot; transfer import copies MSC 1's `world_slots` data verbatim and may use a narrow migration-only reader for a package's active-slot marker/archive when an older package lacks live worlds. MSC 1's raw importer calls `createInitialWorldSlotIfNeeded`; Phase 6 owns the formal replacement. This is a real sequencing tension between phases, not an oversight. **P6.1 (`docs/msc2/worlds/phase6-scope.md`) picks this up**: it reads both import paths above directly and fixes the authoritative reconciliation rule for whatever state they can leave on disk — live folders only, `world_slots` only, or both together.
