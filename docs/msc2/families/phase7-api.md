...
Same methodology `phase6-api.md` §7 established, re-applied here rather than re-derived: the following is the **P7.9 status snapshot**, grounded in what `crates/msc-agent/src/main.rs::build_app()` and `crates/msc-agent/src/cli/mod.rs` mounted at that point. Later Phase 7 steps replaced the placeholder health response and wired additional routes and clients; the current capability state belongs in `docs/msc2/client-capability-matrix.csv`.

- **`agent_status` (P7.9 snapshot).** `build_app()` mounted exactly one of these eighteen routes at that point: `GET /v1/health`, returning P2's canned `demo-card` (`routes/health.rs`'s own doc comment: *"a single canned health card, standing in for real health-check detection"*). It stayed `Implemented` under the phase's route-exists standard, while every other row, including the new `java-runtimes/install` row, was `Planned` for later Phase 7 steps.
- **`desktop_web_status` (P7.9 snapshot)** stayed `Planned` on every row without exception, per `rolling-plan.md`'s Phase 7 preamble — the same blanket rule Phase 6 applied at the time.
- **Desktop/browser and CLI statuses (P7.9 snapshot)** stayed `Planned` on every row because neither retained client had been repointed at these operations yet.
