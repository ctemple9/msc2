...
## P7.36 — Startup-diagnostics vertical slice completed (2026-08-20)

Codex's 2026-08-20 independent REVIEW (recorded in `rolling-plan.md`'s Amendments log) found the second of its two gate gaps here: `scan_paper_soft_failures` had no production caller, its required `analyzePaperPlugins` parser was never ported, and every real modded-crash diagnosis supplied an empty `installed_mods`, so no problem could ever attribute an `installed_jar_stem` — the field the disable/delete repair actions gate on. P7.36 closes this as one end-to-end vertical slice, not a collection of separate fixes.

**Domain boundary correction, not just a build.** Phase 1's own P1.7 step (`rolling-plan-archive.md`) deliberately did not port `StartupCrashAnalyzer.analyzePaperPlugins` — untested by any fixture at the time, and out of that step's own named scope. `msc2-symbol-ledger.csv` never carried a row for it at all (confirmed by grep — only its *caller*, `scanPaperSoftFailures`, was ever rowed, under `target_domain=diagnostics`). That target domain is itself notable: it is **not** one of the nine domains this phase's own "Symbol-ledger rows owned by this phase" table above names (`server-creation`, `java-runtime`, `templates`, `startup-diagnostics`, `components-versions`, `component-version`, `server-installation`, `setup`, `prerequisites`) — yet P7.22 already ported `diagnoseUnexpectedStop`/`scanPaperSoftFailures` (the `diagnostics` domain) into this same phase's `diagnostics.rs`, and P7.36 now ports `analyzePaperPlugins` (a new row, same domain) and builds real subsets of the `mods`/`plugins` domains (`msc_application::add_on_inventory::scan_mods`/`scan_plugins` — filename/manifest inventory only, not the Modrinth/GitHub/Hangar tiering and update-resolution features those domains' full rows describe) alongside them. This was never a formal scope expansion decided up front; it is the practical shape "startup diagnostics" always had to take once diagnosing a real crash meant reading real jars off disk. Recorded here so the boundary is explicit rather than silently drifting: **Phase 7 in practice also owns the `diagnostics` domain in full, and the inventory-reading (not management/update) subset of `mods`/`plugins`.** The symbol ledger's three touched rows (222, 241, 283) and the new `analyzePaperPlugins` row now say this directly.

**What actually changed, by layer:**
- **`msc-domain::crash_analysis`**: `analyze_paper_plugins` (the port), plus a new `PluginEntry` type — deliberately smaller than MSC 1's own `PluginEntry` (no `tier`/`sourceConfig`/online-version fields; confirmed against the frozen `StartupProblemDTO`/`HealthProblemsResponseDTO` contract that none of them are needed anywhere in it).
- **`msc-application::add_on_inventory`** (new file): the real `mods/`/`plugins/` directory scanner. Mods read `fabric.mod.json`/`META-INF/mods.toml` (hand-rolled parsing, matching `ModJarMetadataParser`'s own non-TOML-library approach) with filename-heuristic fallback; plugins are filename-heuristic only — confirmed by reading `refreshDiscoveredPlugins` directly, it never reads `plugin.yml` in its main discovery path, only a separate Modrinth-matching feature does. Malformed archives, duplicate jar stems, and path containment have no MSC 1 oracle behavior at all (confirmed by reading both scanners) — decided for this port, documented in the module's own doc.
- **`msc-application::lifecycle`**: `mark_ready` now runs the Paper-plugin scan exactly once per start (naturally, since `ingest_console_line` only reaches it while `state == Starting`) and calls `scan_paper_soft_failures` for real; `record_stop_diagnostics` now scans real mods instead of passing `&[]`. `ingest_console_line`/`mark_ready` both grew a caller-supplied `now: &str` (the same "caller supplies the clock" precedent `performance_snapshot` already set), rippling into `routes/lifecycle.rs`'s one production call site and five `msc-application/tests/*.rs` files that call either directly.
- **`msc-application::diagnostics`**: a new `remove_repaired_problem`, closing a gap this step's own research found beyond what P7.22 built: `health_repair` verified a repair and rewrote nothing else, so a fresh `GET /v1/health/problems` kept reporting an already-fixed problem. MSC 1 never needed this — it drops a repaired problem from a session-local in-memory array, never touching disk; this headless agent has no such cache and reads the persisted record fresh every call, so removing it from disk is the only way "remove only the repaired problem after verification; preserve it on failure" (this phase's own working exit criterion) becomes real.
- **`msc-agent::routes::health`**: wires the above — `health_repair` now calls `remove_repaired_problem` after a verified repair; its module doc's stale "no mod-jar scanner" note is corrected.

**Verification:** `python3 tools/fixture-runner/run.py --validate-dir fixtures/paper-plugin-crash-analysis --expect 8` and `--validate-dir fixtures/installed-addons --expect 8`; `cargo nextest run -p msc-domain -p msc-application -p msc-agent -E 'test(/crash_analysis|add_on_inventory|diagnostics|lifecycle_state|runtime_diagnostics_routes/)'` plus the wider ripple set (`lifecycle_state`, `command_input`, `lifecycle_with_fake_process`, `java_stop_restart`, `status_metrics`, `health`) — all green. Full detail in P7.36's own "Actual result" in `rolling-plan.md`.

---

## P7.37 — Exact-candidate gate evidence, both synthetic and real (2026-08-20)

P7.37 assembles the evidence the independent REVIEW checks the gate against; it does not decide the gate itself. Two independent bodies of proof, neither substituting for the other:

**1. The committed synthetic smoke (`tools/phase7/phase7-gate-smoke.sh --synthetic`) now proves every review-sensitive path P7.35/P7.36 opened, not just the original six-family happy path.** `fake_provider_server.py` previously served the *real* corpus checksum (Mojang's, Paper's, Purpur's own recorded digest for a *real* jar) alongside a *fake* locally-built jar — exactly the mismatch P7.35's own enforcement now correctly refuses, which is why P7.35's own "noticed but not acted on" note flagged this script as already broken against its own unmodified self. The fix: the fake provider now hashes its own `--server-jar` bytes once at startup and substitutes that real digest into every catalog response it serves (Vanilla's `downloads.server.sha1`, Paper's `downloads."server:default".checksums.sha256`, and a **new** Purpur `/v2/purpur/{version}/latest` metadata route this fake provider never needed before P7.35 added that exact hop in production). A parallel `bad_checksum_<family>` control-marker convention (matching the existing `fail_download`/`fail_install` markers) corrupts one byte of the served payload on request, while metadata keeps advertising the *correct* digest — proving refusal of a real mismatch, not just proving the happy path still works. `FakeServer.java` grew two file-based control signals read relative to its own working directory (always the server's own directory, confirmed against `ProcessSpawnRequest::working_directory`): `smoke-plugin-failure.txt` (prints a Paper enable-error line before "Done" — a soft failure) and `smoke-mod-crash.txt` (prints a line and exits nonzero before "Done" — a hard failure), needing no production wiring at all. With those in place the smoke script now also proves: the interrupted-create directory from its own existing kill-mid-install section is actually gone after restart (P7.33's orphan sweep, not just the operation journal's own reconciled state); a corrupted Vanilla/Paper/Purpur payload against a correct published digest is refused for all three publisher algorithms; a hard modded crash and a successful-start Paper plugin failure both surface through `GET /v1/health/problems`, each attributed to a real installed jar by P7.36's scanner; and a verified `disable`/`delete` repair actually changes the right jar on disk and removes only its own problem — proven with two independent installed plugins so "only," not "all," is the thing actually checked.

**2. `provider-corpus-check.py`'s evidence mode gained a `checksum_verification` field** (`"enforced_by_production"` / `"not_published"`), required to agree with `checksum.matches_provider_published` (`true` ↔ enforced, `null` ↔ not published) — the "failing checker self-tests" this step's own text asked for, proven by a new `evidence-checksum-verification-mismatch` selftest case (16/16 selftest cases pass, up from 15). This closes a real gap the checker's own shape couldn't previously express: `matches_provider_published: true` from P7.28's original capture only meant an independent post-hoc comparison agreed, since production always passed `None` before P7.35 existed. It says nothing about whether *production's own enforcement* was what accepted the bytes.

**3. All six families' real-provisioning evidence was re-captured live** (`docs/msc2/families/provisioning-evidence/*.json`, superseding the 2026-08-19 P7.28 capture in place, not alongside it), through the real, unmodified `msc-agent` binary with no `MSC2_PROVIDER_*_BASE` override — the same "repeat P7.28 through the ordinary CLI" this step's own text asked for. All six created, reached a genuine `Done` ready line, and stopped cleanly. Because P7.35's enforcement is now live in the exact code path these six creates ran through, a corrupted or substituted Vanilla/Paper/Purpur jar would have been refused by `stage_download` before `eula.txt`/`server.properties`/`world_slots` were ever written — each of those three creates succeeding *is* production-level proof its own checksum check accepted the real bytes, independently corroborated by a fresh re-hash of each on-disk jar against a freshly-fetched publisher digest (Vanilla SHA-1, Paper SHA-256, Purpur MD5 — all three matched exactly). Fabric/NeoForge/Forge again published no digest for `jar_provider.rs` to enforce, recorded honestly as `checksum_verification: "not_published"`. Nothing failed; no family needed a "could not be provisioned" report. Version/artifact drift versus the original capture was cosmetic and expected, not a regression: Purpur's own rolling build advanced (2568/2620 → 2622, byte_size shifted by 98 bytes); NeoForge's and Forge's independently-re-downloaded installer jars hashed byte-for-byte identical to the original capture a day-plus earlier; Forge again resolved `65.1.0` through `promotions_slim.json` despite `maven-metadata.xml` under-reporting it (P7.3's original finding, confirmed live again).

**Full verification, this exact-candidate commit:** `python3 tools/phase7/provider-corpus-check.py --selftest` (16/16), `python3 tools/phase7/provider-corpus-check.py --evidence docs/msc2/families/provisioning-evidence` (ok, all six present), `bash tools/phase7/phase7-gate-smoke.sh --synthetic` (`PHASE 7 GATE SMOKE PASSED`), `cargo fmt --all -- --check` (clean), `cargo clippy --workspace --all-targets -- -D warnings` (clean), `cargo nextest run --workspace` (1153/1153 passed, 0 skipped, no leaky). The tri-platform CI leg and the independent REVIEW itself are the two things this step's own text deliberately leaves to Cameron and to Codex — not pre-empted here.

---

## P7.38 — Final production-boundary corrections (2026-08-21)

P7.38 corrects three discrepancies found inside P7.35/P7.36's already-approved checksum and diagnostics boundaries; it adds no new Phase 7 feature.

- **Checksum-bearing providers now fail closed.** Vanilla requires a 40-character hexadecimal SHA-1 from Mojang's version metadata, Paper requires a 64-character hexadecimal SHA-256 from the selected build entry, and Purpur requires a 32-character hexadecimal MD5 from build metadata. Missing, malformed, or unavailable metadata refuses the download. Only Fabric, NeoForge, and Forge retain the explicitly unverified path because their recorded endpoints publish no digest.
- **The two MSC 1 diagnostic windows remain distinct.** The lifecycle buffer retains the latest 400 lines used by `scanPaperSoftFailures` after ready; hard-crash analysis receives only the final 120 lines, matching `diagnoseUnexpectedStop`'s separate source boundary. A regression puts the Paper failure outside 120 but inside 400 lines and proves it is still diagnosed and attributed.
- **Verified repair success includes persistence.** Once disable/delete has been verified on disk, failure to remove and save the repaired problem record is surfaced as an error. The API no longer claims the repair fully succeeded while its persisted diagnostic remains unchanged.

Focused regressions passed (provider/download 35/35, lifecycle/health 25/25, diagnostics 43/43, provisioning/version-change 45/45), the synthetic Phase 7 smoke passed, formatting and workspace clippy were clean, and the final workspace suite passed 1157/1157 with zero failures (one pre-existing leaky test).
