...
stays incomplete outside this CLI and historical-mobile slice.
...
`AppViewModel.swift`, `MSCSettingsView.swift`, and the copied mobile
...
Phase 4's historical client evidence covers the CLI and the former mobile app. Both used bearer
tokens against the same HTTP API the lifecycle routes use.

In scope:

- Issue durable bearer credentials for the CLI and former mobile client.
...
- Fix the copied mobile client's fresh-install empty-token bug from P2.20.
...
1. Initial local owner credential:
   ...
   user's local CLI. This is the root of trust for creating the first mobile-client token.
...
2. CLI-created pairing challenge:
   ...
   for another client. For the historical Phase 4 slice that client was mobile. The challenge is kept in
   agent memory with an expiry and one-use flag, and the CLI prints a pairing
   URI/QR payload.

3. Pairing exchange:
   The mobile client sent the pairing challenge to the agent and received one durable bearer
...
The former mobile client used a platform keychain entry and user preferences
for its selected host. That storage shape is historical evidence only; MSC 2
does not retain the old single global `remote_api_token` account as its only
durable token slot.
...
## Historical Mobile Fresh-Install Bug

P2.20 found a real bug in the copied mobile app:
...
must not prefill `MSC_DEV_TOKEN`. After a successful pairing exchange, the
mobile client saved the returned token under `host-token.<agent-host-id>` and considered that host
paired.
...
This closes only D-012's historical Phase 4 CLI/mobile credential path and the per-host
...
document's per-host CLI/mobile credential model and does not turn named tokens
