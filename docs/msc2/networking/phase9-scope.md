...
and 303; the frozen API baseline and current `/v1` contract; and the historical
MSC 1 route consumers. The listed native SwiftUI, Finder, Keychain, QR, and
...
4. `GET /v1/connectivity`, all Phase 9 HTTP/WebSocket routes, operations and
   ...
   capability matrix records only delivered desktop/web and CLI surfaces;
...
P9.1 does not silently settle D-012. Phase 4’s CLI bearer path is the
...
and adds durable named-token administration. It does not implement
