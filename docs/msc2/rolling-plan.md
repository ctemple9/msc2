...
> **Last updated:** 2026-08-28 (P12.18b built)
...
**What:** Additive to P12.18b's same component: when Server Type is Bedrock, show the oracle's Bedrock branch instead of the Java Software/Source/Crossplay fields — Bedrock Version (via `GET /v1/versions/create?serverType=bedrock`), Max Players, Xbox Broadcast toggle (always available for Bedrock, not gated behind crossplay). MSC 1's Docker Image field is dead code there (commented out) — leave it out here too, not a silent drop since the oracle itself never shipped it live.
**Verify:** `npm run dev`, switch Server Type to Bedrock on the Configure step, confirm the Java-only fields (category/flavor/crossplay) disappear and the Bedrock fields appear with working version fetch; compare to MSC 1 + checklist.
