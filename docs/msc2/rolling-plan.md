...
> **Last updated:** 2026-08-27 (P12.9, P12.10)
...
| **12** | Client redesign (MSC 1 fidelity, refreshed) | **in progress — P12.2b–j, P12.3/a–e, P12.4a–j, P12.5, P12.6, P12.7a, and P12.7b DONE; P12.3f/g, P12.4, P12.7, and P12.9 built, awaiting Cameron's verification** |
...
### P12.9 — Files tab
**Status:** built, awaiting Cameron's verification. Blocked on a real gap, same shape as P12.3's Players tab: `GET /v1/files` and `GET /v1/files/read` were frozen in the API contract since Phase 11 (`ServerFilesResponseDTO`/`ServerFileReadResponseDTO`/`ServerFileItemDTO` all already existed) but had **no backend at all** -- no route, no application function, no domain logic; `crates/msc-agent/src/routes/mod.rs` never even had a `files` module. Confirmed with Cameron (not silently expanded): he chose to build the backend inline as part of this same step rather than split it into precursor steps, since both routes were already fully specified in the frozen contract (unlike P12.3's mutation actions, which needed new contract fields) -- a bounded, already-scoped port, not new API design.

Built `crates/msc-application/src/server_files.rs` (`browse_directory`/`read_previewable_file`, 12 unit tests against `FakeFileSystem`) and `crates/msc-agent/src/routes/files.rs` (HTTP status mapping, 8 route tests), a direct port of MSC 1's `wireSkinAndFileProviders`' `filesProvider`/`fileReadProvider` (`AppViewModel+APIWiringContent.swift:170-275`, real backend logic, not the client-only `ServerFilesTabView.swift` scan it superficially resembles): same previewable-extension allowlist (txt/log/yml/yaml/json/properties/sh/cfg/conf/toml/ini/md), same dirs-first case-insensitive sort, same escape guard (ported onto the existing `msc_infrastructure::path_safety::safe_path` primitive rather than reimplemented), same 512 KiB read cap with UTF-8-then-Latin-1 decode fallback, same admin-only gate (`require_permission(..., PermissionCategoryDto::Admin)` -- previewable content can carry secrets, e.g. `rcon.password`). One deliberate, documented deviation: the oracle's own HTTP layer never gives a path-traversal escape on `/files/read` a real status code (falls through its switch to a bare 500 default) and the frozen contract doesn't reserve one either; this port folds that case into 404 instead, since hiding the sandbox boundary is safer than a client-input-validation failure reading as a server error, and it's a defense-in-depth path a real client should never hit. `python3 tools/api-contract-check.py --v1-summary` still reports the same route/namespace totals (implementing a route already in the frozen contract adds nothing to count).

Client: `clients/desktop-web/src/lib/sections/files/` (`FilesSection.svelte`, `FilePreviewSheet.svelte`, `model.ts`) ports `ServerFilesTabView.swift`'s Server Root breadcrumb, Folders/Files divided `ListRow` lists (folder rows get a trailing chevron, file rows don't -- matches the real `~/Documents/MSCSS/Tabs` Files screenshot, not just the oracle source), and read-only preview (`FilePreviewSheet`, using `Sheet` + `--msc2-font-mono`). Also added `PlatformAdapter.revealInFileManager` (`clients/desktop-web/src/lib/platform/`) and a new Tauri command `reveal_in_file_manager` (`clients/desktop-web/src-tauri/src/lib.rs`, mirrors `open_external_url`'s shape exactly -- one platform-dispatched shell-out, `.spawn()` not `.status()` since file-manager reveal exit codes are notoriously inconsistent across platforms, especially Windows' `explorer /select,`) for "Show in Finder." Gated on `hostId === 'local-agent'`: a remote host's server files have nothing local to reveal, so the button is disabled with an explanatory `title` for a remote connection, and the browser's own adapter always falls through to a `revealNotice` telling the user this needs the desktop app -- the "web fallback marked" the step's own `What:` line calls for.

**Deliberately out of scope, not silently dropped:** the oracle's `TextPreviewSheet` also supports editing and saving a file in place. There is no write route anywhere in the frozen contract -- not even a reserved-but-unimplemented one, unlike browse/read. Adding one is a real contract-amendment decision (new DTO, new route, an "are you sure" model for a change that can break the server), out of this step's scope; left deferred like `copyPlayerData` was in P12.3, not built and not fabricated.

**Not done in this step, flagged instead of silently edited:** `App.svelte`'s `sections` array (an explicitly off-limits shared foundation file for this parallel worktree) still has no `files` entry, so the tab doesn't appear in the running shell yet and `FilesSection.svelte` isn't in the production Vite bundle. Add:
```js
{
  id: 'files',
  label: 'Files',
...
```
**Found, not fixed (pre-existing, unrelated):** `python3 tools/phase6/capability-matrix-check.py` already reports 15 contract operations with no matrix row at all (auth/pairing routes, the P12.7c catalog-project-detail routes, the P12.3 player-mutation routes, `/v1/worlds/convert/formats`) -- pre-existing debt from several earlier phases, not introduced here; left alone rather than folded into this commit. Two `npm run test:unit` failures are also pre-existing and unrelated: `tests/navigation/navigation.test.ts`'s shell-boot assertion expects a `client.getCapabilities()` string no longer present in `App.svelte` (untouched by this step, last changed in P12.7), and `tests/e2e/tauri-linux/native-renderer.test.ts` is a WebdriverIO/mocha suite that only runs correctly under its own `test:e2e-tauri-linux` script, not plain `vitest run`.
**Files:** `crates/msc-application/src/server_files.rs` (new), `crates/msc-agent/src/routes/files.rs` (new), `crates/msc-agent/src/routes/mod.rs`, `crates/msc-agent/src/main.rs`, `clients/desktop-web/src/lib/sections/files/` (new), `clients/desktop-web/tests/screens/files.test.ts` (new), `clients/desktop-web/src/lib/platform/`, `clients/desktop-web/src-tauri/src/lib.rs`, `clients/desktop-web/tests/tauri/platform-boundary.test.ts`, `clients/desktop-web/package.json`, `docs/msc2/client-capability-matrix.csv`
**What:** Build the previously-missing `GET /v1/files`/`GET /v1/files/read` backend against the already-frozen contract, then rebuild the Files tab against it — Server Root breadcrumb, Folders + Files divided lists, Show in Finder (desktop; web fallback marked), read-only file preview. Reference MSC 1 `ServerFilesTabView`/`AppViewModel+APIWiringContent.swift` and the Files screenshot.
**Verify:** `cargo nextest run -p msc-application --lib server_files::`; `cargo nextest run -p msc-agent --bin msc files`; `cargo clippy -p msc-agent -p msc-application --all-targets -- -D warnings`; `cargo fmt --check`; `npm run test:screen-files`; `npm run test:tauri-boundary`; `npx svelte-check --tsconfig ./tsconfig.json` (only the same 7 pre-existing unrelated errors); `npx prettier --check` on the new/changed files; `npm run build`. Real verification is Cameron's own, **after adding the `files` entry to `App.svelte` above**: `npm run dev`, open Files, browse + preview a text file, try Show in Finder; compare to MSC 1 + `~/Documents/MSCSS/Tabs` + the antiAIslop checklist.
**Commit:** `P12.9: rebuild the Files tab`
**Batch:** solo

### P12.10 — Console (docked, full behavior)
**Status:** awaiting verification (2026-08-27)
**Files:** `src/lib/components/shell/ConsoleDock.svelte`, `src/lib/sections/console/model.ts` — this step's own `Files:` line named `src/lib/sections/console/`, but that folder holds only the legacy pre-redesign full-page `ConsoleSection.svelte`, which the new tab strip (`primaryTabs.ts`) no longer exposes. The real docked console MSC 1 fidelity requires is `ConsoleDock.svelte` (rendered by `ApplicationShell` on every tab), whose own header comment already flagged "becomes real in P12.10" — built there instead, plus additive small edits to `ApplicationShell.svelte` (new optional `api` prop, forwarded to `ConsoleDock`) and `App.svelte` (passes `api={screenApi}` into `ApplicationShell`) so the dock could reach the agent at all; both cleared with Cameron first since they're shared-foundation files a parallel worktree was told not to touch unasked. `ConsoleSection.svelte` itself was left untouched.
**What:** Complete the docked console — filter chips (All/Server/Plugins/Warnings/Controller/Commands/Custom), search, buffered log, command input + Send, collapse/expand, copy/clear. Reference MSC 1 `ConsoleView` and the console in `~/Documents/MSCSS/Main View`. Filter-chip categorization (Server vs. Plugins) is inferred client-side from bracketed tokens in the raw line text, the same heuristic MSC 1's `ConsoleLineParser` uses, since the agent's `ConsoleLineDTO` carries no `tag` field; `level` is likewise inferred from text since the agent never sets it (see `crates/msc-agent/src/routes/lifecycle.rs`). The buffered log polls `GET /v1/console/tail` every 2s rather than the agent's real `GET /v1/console/stream` WebSocket — simpler and needed no further foundation edits; upgrading to the live socket is a reasonable follow-up, not required by this step.
...
**Status:** awaiting verification. Ported all 42 of `MinecraftCommandRegistry.swift`'s command definitions (not ~40 — exact count once transcribed) into `model.ts`, plus `commandsFor`, `hasRequiredArgs`, `commandSyntaxHint`, `buildCommand`, and the suggestion engine. `CommandPaletteSheet.svelte` is one component with internal browse/builder state (matching `CommandPaletteView`'s own single-view-with-a-mode-enum shape, not the two-separate-sheets pattern `ComponentsSection`'s browse→detail drill-down uses) — `Sheet`'s title swaps between "Command Palette" and `/name` per mode, and "Back" is a plain secondary button in the body (`Sheet.svelte` has no header slot for it, same adaptation `WorldConversionWizard` already made for its own step-back buttons).

One real bug found in the oracle while porting the suggestion engine, fixed rather than carried over: `MinecraftCommandRegistry.suggestions` tokenizes with `split(omittingEmptySubsequences: false)`, which keeps the empty string a trailing space produces as if it were a real, already-filled argument token. That makes the *first* space after a command name (or after finishing an argument) point suggestions at the next-plus-one slot with a doubled space — e.g. typing `/tp ` would suggest for "destination player" as `/tp  Steve` (two spaces), not `/tp Steve` for "target player". This port tokenizes on non-empty tokens instead, which is simpler than the oracle's version and gives the behavior the feature obviously intends. Documented in `model.ts`'s own comment; covered by a dedicated test (`/tp ` → correctly suggests the *first* argument).
...
**Status:** awaiting verification. Simpler than this step's own plan text assumed: `ApplicationShell` already received `servers`/`activeServerId` as props (for `DetailsHeader`), so `serverType` needed no new prop threading at all through `App.svelte` — just one line at its existing `<ConsoleDock>` call site (`serverType={activeServer?.serverType}`). `App.svelte` itself needed zero changes; dropped from the Files list below versus what was planned.

Added a palette-trigger icon-action (list-in-rectangle glyph, left of copy/clear, matching MSC 1's exact button order) that opens `CommandPaletteSheet`; its `onUse` just sets `command`, reusing the dock's existing `send()`/`POST /v1/command` path untouched. Autocomplete runs P12.10a's `commandSuggestions` reactively off `command` and renders a horizontal pill strip above the input (`ConsoleView.autocompleteStrip`'s shape) when non-empty; clicking a pill fills `command`, same no-auto-send behavior as the palette. `ConsoleDock` now also polls `GET /v1/players` on its existing 2s tail-poll cycle (added to `livePaths`) to feed both the palette's player chips and autocomplete's player-argument matching.

**Found, documented, not fixed (pre-existing, out of this step's scope):** `GET /v1/players` (`crates/msc-agent/src/routes/players.rs`) is Bedrock-only — a Java active server always gets back `{players: [], note: "not_bedrock"}`. So today, a Java server's command palette/autocomplete has no online-player roster to offer and correctly falls back to plain text entry for player arguments (exactly MSC 1's own zero-players fallback) rather than fabricating one. A live Java online-roster route doesn't exist in the contract yet.

**Deliberately out of scope, not silently dropped:** MSC 1's autocomplete on/off toggle (`@AppStorage("msc_autocomplete_enabled")`, a `wand.and.rays` button next to the console search field) — a separate, smaller preference this step's own plan text never named.
**Files:** `src/lib/components/shell/ConsoleDock.svelte`, `src/lib/components/ApplicationShell.svelte`, `src/lib/sections/console/model.ts`
