...
### P12.187 — Complete safe Chunker world conversion
**Status:** planned
**Files:** `crates/msc-agent/src/routes/worlds.rs`, `crates/msc-application/src/world_conversion.rs`, `crates/msc-application/src/worlds.rs`, `crates/msc-domain/src/`, `crates/msc-infrastructure/src/`, `crates/msc-api/src/`, `clients/desktop-web/src/lib/sections/worlds/WorldConversionWizard.svelte`, `clients/desktop-web/src/lib/sections/worlds/model.ts`, `docs/msc2/api-contract/openapi.json`, `clients/desktop-web/src/lib/api/generated.ts`, `crates/msc-agent/web-ui/`, `docs/msc2/rolling-plan.md`
**What:** Finish the Chunker feature as one safe, guided Java↔Bedrock workflow. Add agent-owned Chunker acquisition from the official GitHub release with version/status reporting, temporary-download-and-atomic-install behavior, Java resolution, supported-format discovery, progress, and actionable failure messages; do not accept arbitrary host paths or silently download an untrusted artifact. Expand the conversion wizard with an explicit Download Chunker path, source/target and format guidance, a clear disclaimer that conversion is not perfectly lossless, a default new-slot destination, and progress/recovery states. Strengthen the conversion transaction so the source is never modified, the target server must be stopped, a verified pre-conversion backup is mandatory (backup failure aborts before target mutation), all output is staged outside the target, cancellation and failures clean up incomplete work, and activation happens only after validation. Add format-aware post-conversion validation beyond wrapper folders: confirm the expected Java or Bedrock world root, parse the target-format `level.dat`, verify non-empty world storage (`region`/dimension data for Java or Bedrock `db` data), inspect player data when present without rejecting a world that has no players yet, reject mixed or malformed source-format markers where they indicate the wrong target, and report warnings separately from a hard validation failure. A failed validation must leave the original source and target active world intact; a successful conversion creates the target slot and activates it only after the validation gate. Reconcile the API contract and regenerate the embedded web bundle. Use the existing conversion fixtures and add no tests without Cameron’s approval; Cameron’s manual verification must cover fresh install, Java→Bedrock, Bedrock→Java, missing/corrupt Chunker output, failed backup, cancellation, and a world with and without player data.
