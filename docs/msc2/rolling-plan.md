...
**Correction:** The launch test now constructs its expected JAR path with the same component-wise joins as acquisition before converting it to a process argument, so separators follow the target platform on Unix and Windows.
