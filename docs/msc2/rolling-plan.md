...
> **Last updated:** 2026-08-28 (P12.18c built)
...
**Status:** not started
**Files:** `clients/desktop-web/src/lib/sections/fleet/wizard/NetworkStep.svelte` (new), `clients/desktop-web/src/lib/sections/fleet/wizard/AddServerWizard.svelte`
**What:** Port the shared Network step (used by both Fresh and Import paths — build it generically now so P12.18h can reuse it unchanged): Port Forwarding vs Tunnel (playit.gg) path cards, then the port fields that vary by context (Java port + Bedrock/Geyser port when crossplay is on; Bedrock server port when the server type is Bedrock; single Java port otherwise), plus a link into the already-built router port-forward guide content (P12.16, or its current pre-P12.16 location if that step hasn't landed yet — check `docs/msc2/rolling-plan.md`'s current P12.16 status before wiring the link).
**Verify:** `npm run dev`, walk Network for a Java server, a crossplay-enabled Java server, and a Bedrock server, confirm the right port fields appear each time; compare to MSC 1 + checklist.
