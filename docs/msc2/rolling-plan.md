...
> **Last updated:** 2026-08-28 (P12.18e built)
...
**Status:** not started
**Files:** `clients/desktop-web/src/lib/sections/fleet/wizard/AddOnsStep.svelte` (new), `clients/desktop-web/src/lib/sections/fleet/wizard/AddServerWizard.svelte`
**What:** Port step 5 Fresh/Add-ons, shown only when the chosen Java flavor has an add-on kind (mirrors `hasAddOnsStep`): empty state with Browse Modrinth (reuse `PluginBrowserSheet.svelte`) and Import (reuse `ImportModpackSheet.svelte`) tiles; once staged, a list of pending add-ons with remove. Read how `applyStagedAddOn`'s modern equivalent actually threads a pre-create staged list into `POST /v1/servers/create` (likely `stagedModpackUploadId` for a pack, or a separate post-create install call for loose plugin picks — confirm against the real route before wiring, don't assume) and wire accordingly.
**Verify:** `npm run dev`, reach Add-ons on a Paper/Fabric server, browse and stage a plugin/mod, confirm it appears in the pending list and can be removed; compare to MSC 1 + checklist.
