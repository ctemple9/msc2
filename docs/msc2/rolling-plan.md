...
**Batch:** stop-after
**Confirmed on real hardware:** Cameron's next run — `powershell -ExecutionPolicy Bypass -File tools/phase4/windows-service-lifecycle.ps1 -ServerDir C:\msc2-paper` against a real, freshly-generated Paper 1.21.11 server — completed the full import → start → console-ready → command → console-tail-verify → client-exit-survival slice and printed `checkpoint recorded at ...` / `sign out of Windows, sign back in, then rerun this exact command`. This is P4.24's own sign-out/sign-in survival checkpoint; that half of the proof is still outstanding pending Cameron actually signing out and back in and re-running the same command to resume it.
