...
> ## STATUS: Phase 4 in progress — macOS LaunchDaemon (P4.22) and iOS (P4.20) proven for real; Linux's first real run found and fixed a real SELinux bug (P4.34), awaiting Cameron's re-run; Windows still needed
> **Next move:** VERIFY P4.29–P4.34 and the P4.20 iOS fix. Then: Cameron re-runs `sudo env "PATH=$PATH" tools/phase4/linux-service-lifecycle.sh --server-dir <path>` on the Fedora box to confirm P4.34's fix gets the full lifecycle slice passing. Then P4.24 (Windows Service + sign-out check, has Bootcamp Windows available).
> **Repo:** https://github.com/ctemple9/msc2 · CI green on macOS, Linux, and Windows as of commit `7a8ea59` ([run 30777077981](https://github.com/ctemple9/msc2/actions/runs/30777077981)) for the Rust side; the iOS fix (P4.20) is Swift-only and confirmed with a real `xcodebuild` success, not covered by the Rust CI matrix. The prior "CI green" claim on this line was never actually checked against a real Actions run — Claude's Phase 4 gate review found it red on all three legs; see the amendments log for that, and for the P4.31→P4.33 debugging trail (a real `launchctl start`/`stop` bug in the shipped Rust code, only found by running the real integration script against a real server, not by any unit test). P4.34 is the same shape of finding, on Linux: `crates/msc-platform-linux/src/service.rs` itself looks correct against real `systemctl` semantics (unlike `launchctl`, `systemctl` takes the same unit-name argument for every subcommand), but the integration script's own use of `/tmp` tripped an SELinux denial on Fedora that Debian, this step's original target, can't surface at all.
...
**Batch:** stop-after
**Not yet ruled out:** only the `RUN_DIR` denial has been seen and fixed so far — the credential-helper socket/service (`/run/msc2`, `/var/lib/msc2/credentials`) hasn't been exercised yet since the script never got that far. Those paths are created the same way (plain `mkdir -p` under the root shell) but `/run` and `/var/lib` don't carry the same `/tmp`-specific user-domain transition rule, so they may be fine — but that's an expectation, not something confirmed yet. If Cameron's next run hits a similar denial further in, same isolate-and-check-`ausearch`-first method applies.
