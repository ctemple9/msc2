...
**Status:** not started
**Files:** `src/lib/sections/setup/AgentSetupSection.svelte`, `crates/msc-agent/src/cli/service.rs`, `crates/msc-agent/tests/cli_service.rs` (new)
**What:** Two parts, per the 2026-08-27 design discussion. **(1) Backend gap, wired inline (P12.9 precedent):** `crates/msc-agent/src/cli/service.rs`'s `run()` currently always returns `"service management is modeled but not executable yet"` regardless of the subcommand. The real per-platform logic it should call already exists and is hardware-proven (Phase 4) — `clients/desktop-web/src-tauri/src/lib.rs:480`'s `service_manager()` already selects the right concrete manager (`MacosLaunchdServiceManager` / `WindowsServiceManager` / `LinuxSystemdServiceManager`) and calls `.execute(ServiceManagerCommand::...)` on it for Tauri's native commands. Give the CLI the same selection + execute call so `msc service {status,start,stop,install,uninstall}` actually works from a terminal on every platform; there is no `repair` verb at this layer (Tauri's "Repair" is `uninstall` + `install` + `start` composed, per `src-tauri/src/lib.rs:432`, not a primitive). **(2) Frontend redesign:** rebuild `AgentSetupSection.svelte` on the S0 primitives every other rebuilt Phase 12 screen already uses (`Button`/`Card`/`Badge`/`StatusDot`/`Field` from `components/base/`), replacing the pre-redesign `ActionButton`/`SurfaceCard`/`StatePanel`/`ScreenHeader`/top-level `StatusBadge`. Head it with the host's name (`"‹Host name› agent"`, matching the existing breadcrumb pattern) since it must now name which host it's talking about. Branch its body three ways: **Tauri** keeps today's real Install/Start/Stop/Repair buttons, restyled only; **browser on the host machine** (detected by the connected host's `baseUrl` resolving to loopback — an unspoofable same-machine signal, no heuristic needed) replaces the old "install the headless package" dead end with a copy-pasteable terminal-command panel using the now-real CLI (`msc service status/start/stop --service-name msc-agent`, plus the two-command uninstall→install sequence for repair), each with a Copy button; **browser on a different machine** shows distinct copy ("you're not on this host") with no service-control content at all, since none of it applies from there.
**Verify:** Backend: `cargo nextest run -p msc-agent --test cli_service` (new test asserting `msc service status/start/stop/install/uninstall` execute against a fake `ServiceManager` instead of erroring, on all three platform branches via `#[cfg]`-gated fakes or an injectable manager). Frontend: `npm run dev`, stop the local agent and observe the Tauri-branch buttons still work; open the same state in a browser at `localhost` and confirm the copyable command panel (verify the printed commands actually work when pasted); open it via a non-loopback address and confirm the "wrong machine" copy. Compare to checklist (no MSC 1 oracle — this screen has none).
