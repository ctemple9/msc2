...
**P3.14 amendment — algorithm-aware checksum contract (P7.35, 2026-08-20).** P3.14 built `stage_download` against a single hex SHA-1, the nearest MSC 1 precedent available at the time (`ResourcePackManager.sha1Hex`) — no real Phase 7 provider evidence existed yet to know whether that was the right shape. Codex's 2026-08-20 Phase 7 review found the consequence: every real `jar_provider.rs` download call passed `None` as the expected checksum, so `stage_download`'s own verification never actually ran against a real provider's published digest — corrupt or substituted bytes from Mojang, Paper, or Purpur were accepted and installed. Real Phase 7 evidence (`corpus/providers/README.md`'s P7.28 finding, confirmed against the live APIs again by P7.35) shows the six server-jar providers genuinely don't agree on an algorithm: Mojang's per-version metadata publishes SHA-1, Paper's fill v3 build entries publish SHA-256, and Purpur's per-build API publishes MD5 only — Fabric's composed download endpoint and NeoForge's/Forge's Maven publish no digest at all. P7.35 amended `stage_download`'s contract from a bare `Option<&str>` SHA-1 to `Option<&ExpectedChecksum>`, where `ExpectedChecksum` names which of the three algorithms its hex digest is in, and wired the digest extraction into every Vanilla/Paper/Purpur download call site in `jar_provider.rs` — extracted from the exact provider response already used to choose the download (Mojang's and Paper's digests ride along in a response the download path already fetches; Purpur's needed one new hop, since its own download URL never resolves a build id). Fabric/NeoForge/Forge stay explicitly unverified, matching what their real endpoints actually publish (nothing) rather than inventing a digest. `None` still means exactly what it always meant under P3.14: no digest to verify against, staged and moved unverified.

**P3.14 fail-closed clarification (P7.38, 2026-08-21).** `None` is valid only when the provider endpoint genuinely publishes no digest: Fabric, NeoForge, and Forge. Vanilla, Paper, and Purpur are checksum-bearing providers, so missing, malformed, or unavailable checksum metadata is an error and the download is refused; those cases may not be converted to `None`. This closes the ambiguity left by P7.35's first implementation, which validated mismatching hashes but still allowed missing or malformed hashes to silently become an unverified download.
...
| Real per-domain download workflows (Paper/Xbox-jar/plugins/modpacks) | No — only the shared staging primitive (P3.14, amended by P7.35 to be algorithm-aware) is | Phases 7–9 |
