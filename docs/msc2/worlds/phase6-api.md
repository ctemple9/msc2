...
Twelve new operations close the gaps `fixtures/world-mutations`, `fixtures/world-archive-safety`, `fixtures/backups`, and `fixtures/world-conversion` characterized but the baseline never exposed. (A thirteenth, `POST /v1/worlds/copy`, was proposed here originally but removed post-review — see §9: it duplicated `POST /v1/worlds/replace`'s corrected, real semantics exactly. A fourteenth, `POST /v1/worlds/replace-active-world`, was *added* post-review instead — see §10: the gate review that caught §9's naming trap also caught that fixing the trap left `replaceWorld`'s own direct-live-world capability with no route at all.)
...
| `POST /v1/worlds/replace-active-world` | `replaceActiveWorld` | `AppViewModel+WorldManagement.swift::replaceWorld` — direct live-world replacement, added post-review (§10) | **async** — `WorldReplaceActiveResultDTO` |
...
Twelve routes, thirteen operations (`staged-uploads/{id}` and the others are one operation each — `tools/api-contract-check.py`'s `EXPECTED_TOTAL` counts the operation, not the route: 88 baseline + 5 P2.8 + 13 P6.8/P6.34 = 106). **`EXPECTED_TOTAL` itself is not yet updated** — `tools/api-contract-check.py` isn't in P6.34's own Files list or Verify command, so this note only records the true count here; bumping the constant is left as an immediate P6.36 (or sooner) correction, flagged in the P6.34 report.
...
This matrix is a snapshot, not a one-time artifact — `tools/phase6/capability-matrix-check.py` (SS8) keeps it honest against `openapi.json`'s actual operation set on every run, and later phases that wire real service logic, CLI commands, or desktop/browser screens are expected to flip the relevant cells from `Planned` to `Implemented` as part of *that* phase's own commit, the same way the earlier client wiring steps did for the rows they touched.
...
## 10. Post-Phase-6-gate-review addition: `POST /v1/worlds/replace-active-world` (2026-08-16)

§9's correction fixed `POST /v1/worlds/replace`'s naming, but left a gap it didn't itself close: once `{slotId, sourceSlotId}` was correctly attributed to `copySlotIntoExisting`, `AppViewModel+WorldManagement.swift::replaceWorld` — MSC 1's own direct, live-world replacement capability (clear the live world's folders and install a fresh world, a backup ZIP, or an existing folder in their place, under a new level name) — had no route mapped to it at all. The Phase 6 gate review caught this and asked for `worlds::replace_world` (P6.33) to be made transactional first; this step exposes it.

- **A separately-named route, not a reused one.** `POST /v1/worlds/replace-active-world` / `replaceActiveWorld`, distinct from `POST /v1/worlds/replace` / `worldSlotReplace`(§9) — the two operations share a verb but nothing else (one never touches the live world, the other never touches a saved slot).
- **Bounded staged upload only, never a server-local path.** `WorldReplaceActiveRequestDTO` is `{newLevelName, stagedUploadId?}`. A present `stagedUploadId` must have been begun with `purpose: "active-world-replace"` (the staged-upload purpose enum, SS4, gains this second value) and is redeemed exactly once, the same "missing/expired/wrong-purpose staged id is a plain 404" contract `import` already established; an absent `stagedUploadId` replaces with a fresh (empty) world. There is deliberately no way to name an arbitrary path on the agent's own filesystem — `worlds::WorldReplaceSource::ExistingFolder` (the one variant that *would* accept a bare path) is reachable only by in-process callers, never by this route.
- **Always async, always a mandatory safety backup, guard-ordered like `restore`.** Folder moves and (for an uploaded source) zip staging are real filesystem work, the same class as `activate`/`backups/restore` — this route always returns `{result: "replace_started", operationId}` and does the real work on a spawned task. It follows `POST /v1/backups/restore`'s guard order, the closer existing analog (mandatory backup + transactional swap + cancellation), rather than `activate`'s: a running server is refused up front (`409 conflict`), then the staged upload (if any) is redeemed up front (`404` if missing/expired/wrong-purpose), then the journaled operation begins and the mandatory, verified pre-replace backup and transactional swap (`worlds::replace_world`, P6.33) run in the background, participating in the ordinary permission/audit/exclusivity/cancellation model every other Phase 6 mutation already has.
- **`newLevelName` must match the source's own folder name for a non-fresh replacement.** `worlds::replace_world` only ever writes `level-name` into `server.properties` (P6.33's `apply_world_identity`) — it does not rename any folder on disk. For `BackupZip`/uploaded sources this is an existing P6.33 contract, not a new one this step introduces; the CLI's own `--help` text says so.
- **Desktop/web stays Phase 11; former mobile coverage is historical.** MSC 1's former mobile client had no direct-live-replacement screen of its own (only macOS's `AppViewModel+WorldManagement.swift` does), so no current mobile capability row is needed — this is a scope boundary, not a D-023 "Intentional exception."
