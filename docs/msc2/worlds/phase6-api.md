...
- **Desktop/web stays Phase 11; iOS stays `Planned`, not an exception.** MSC 1's iOS client has no direct-live-replacement screen of its own (only macOS's `AppViewModel+WorldManagement.swift` does), so there is nothing for the copied iOS client to be repointed at yet — this is a scheduling fact, the same as every other Phase 6 client row, not a D-023 "Intentional exception."
