...
EXPECTED_TOTAL = 131  # 88 baseline (P0.23 --total) + 5 P2.8 + 12 P6.8 (copyWorldSlotContent removed post-review; folded into /v1/worlds/replace) + 1 P6.34 (POST /v1/worlds/replace-active-world, replaceActiveWorld) + 1 P7.9 (POST /v1/java-runtimes/install, installJavaRuntime, D-006 addendum) + 3 P8.9 (POST /v1/modpacks/inspect, POST /v1/modpacks/import, POST /v1/modpacks/{operationId}/manual-file, docs/msc2/addons/phase8-api.md SS3) + 5 P11.21 (browser/desktop pairing and browser session routes) + 4 P11.24 (content catalog and structured guide routes) + 2 P11.29i (GET/POST /v1/config/servers-root, missed since Phase 11) + 4 P12.2g (player delete, offline-UUID migration, custom-UUID migration, and duplicate routes) + 1 P12.3d (POST /v1/players/identify, Bedrock gamertag assignment) + 1 P12.3a (POST /v1/session-log/clear, clear session log) + 1 P12.4a (GET /v1/worlds/convert/formats, expose Chunker's supported conversion formats) + 1 P12.4b (POST /v1/worlds/{slotId}/thumbnail, set a world slot thumbnail) + 2 P12.7c (GET /v1/catalog/projects/{projectId} and GET /v1/catalog/projects/{projectId}/versions)
